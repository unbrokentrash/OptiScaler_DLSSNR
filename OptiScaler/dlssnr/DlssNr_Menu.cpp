...
void RenderMenu(Config* config, float menuResScale)
{
    ...
    if (auto ch = ScopedCollapsingHeader("DLSS Neural Rendering"); ch.IsHeaderOpen())
    {
        ...
        HelpMarker("Hands the model a picture DLSS has already resolved, instead of the raw frame."
                       ...
                       "\n\nIt cannot damage the frame, and that took a fix to be true. The"
                       "\ncomposition does NOT normally add an edit onto the frame -- it hands back the"
                       "\nMODEL'S PICTURE with its luminance re-anchored -- so the first version of"
                       "\nthis put the second DLSS's reconstruction straight into the player's frame."
                       "\nThat is replacing the picture, not cleaning the model's input, and it is"
                       "\nwhat \"it seems to replace the NR\" was."
                       "\n\nThe resolve is now told when the model's input was substituted, and then"
                       "\nrebuilds the frame's own proxy and carries only the model's DIFFERENCE onto"
                       "\nit. This pass is on both sides of that subtraction and cancels. The upscaler"
                       "\ndownstream gets the game's own jittered frame with the model's edit on it."
                       "\n\nThe same fix applies to the frame-averaging below, which had the same hole."
                       "\n\nOne view is deliberately not covered: reversible \"replace\" bypasses the"
                       "\ncomposition on purpose to show the model's raw answer. With this on, that"
                       "\nview shows the model's answer on a DLSS-resolved frame -- true about what the"
                       "\nmodel returned, no longer true about what it did to the GAME'S frame. Turn"
                       "\nthis off to use replace as an A/B, or read the edit in Debug view 3."
